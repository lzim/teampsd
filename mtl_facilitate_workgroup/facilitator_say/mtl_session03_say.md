...
So, in SPParams, cell D2 is the definition for the patient “Start Rates.” Cell E2 describes which patients are included in the Start Rate Estimates and additional details.
...
5. Are there any team data table values you would like to explore in the team data UI?
  ...
  c.	For example, if our team wants know how long patients are waiting to start Medication Management, we would click on “MMParams” and scroll down to row 6. Columns B thru E show the average wait time for patients diagnosed with Alcohol Use Disorder (AUD), Depression (DEP), Opioid Use Disorder (OUD), and All Other Diagnoses (Other). [scroll down to show column footer titles]
...
1. We encourage you to look around in the team data table before next time. Test whether you can find a piece of data you decide to look for. Just take a minute, if you can, to become familiar with the format and content of the data table.
  •	A reminder, you can click through the Params tabs, 1 for each module [click on each Params tab], and dig in to the details of how each parameter is calculated in the text columns to the right of the numerical data.
...
3. We will aggregate the responses and consider them together in the next session to help us choose which MTL module we'll use for the next several sessions of Modeling to Learn
