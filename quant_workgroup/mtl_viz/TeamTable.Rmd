...
def_vec <- c(rep(uni_colD, group_pos[1:length(uni_colD)]), rep("",11))
...
mcc <- sppar1 %>%
  mutate(group = group_vec,
         ...
         #to = ifelse(grepl("to",col_lab),'step','measure')
         ) %>%
  select(group, col_lab, X__3, def) 


rw_ord1 <- c("New Patient Start Rate (mean)(pts/wk)", "New Patient Wait Time (median)(wks)",
             "High Risk Flag Rates (mean)(pts/wk)", "Time to Unflag High Risk Patients (median)(wks)", "Engagement Time Before Ending (median)(wks)", "High Symptom Proportions %",
...
col_ord2 <- c("Time from Flag to Clean Transfer (median)(wks)",
             "Engagement Time before Step up/down (median)(wks)", 
             "Wait Times (median)(wks)")
rw_ord2 <- c("GMH to PCMHI", "GMH to SMH", "GMH to Inp", "PCMHI to SMH", "PCMHI to GMH",
             "PCMHI to Inp", "SMH to GMH", "SMH to PCMHI", "SMH to Inp")
mcc2 <- mcc %>%
...
htmlTable(mcc1[,-5], header = c(rep("",4)), rnames = FALSE, css.cell = "padding-left: 1em; padding-right: 1em;", caption ="Team Data", align = "lccc")

```

```{r sp_table2, echo=FALSE}

htmlTable(mcc2, header = c("", names(mcc2[-1])), rnames = FALSE, css.cell = "padding-left: 1em; padding-right: 1em;", caption ="Team Data", align = "lccc")
...
htmlTable(mcc1, header = c(rep("",5)), rnames = FALSE, css.cell = "padding-left: 1em; padding-right: 1em;", caption ="Team Data", align = "lcccl")
