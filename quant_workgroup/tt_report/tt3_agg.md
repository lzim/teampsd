...
aggpar.table.body <- aggpar[c(1,5,3,4),] %>%
  ...
  mutate(Variable = replace(Variable, Variable == "Median Engagement", "Engagement Duration (median)"))
...
aggpar.test <- aggpar.table.head[,1:5] %>%
  rbind.data.frame(aggpar.table.body.shrt) %>%
