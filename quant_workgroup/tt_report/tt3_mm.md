...
mmpar <- read_excel("ModelParameters.xlsx", sheet = "MMParams", col_names = paste("X",1:6, sep = "__"), range = "A1:F16")

mmpar.table1 <- mmpar[c(10, 15, 12, 5, 3, 1,7, 13, 14),] 
...
mmpar.table <- mmpar.table1 %>%
  mutate(X__5 = replace(X__5, X__1 == "Appointment Supply (75th percentile)", 
         ...
         X__1 = sub(" in MM", "", X__1), 
         X__1 = replace(X__1, X__1 == "True Missed Appointment %", "True Missed Appointment "), 
         X__1 = paste0(X__1, stu)) %>%
  mutate_at(vars(2:5), funs(as.numeric(.))) %>%
  mutate(X__2 = ifelse(grepl("%", X__1), X__2 *100, X__2),
         X__3 = ifelse(grepl("%", X__1), X__3 *100, X__3),
         X__4 = ifelse(grepl("%", X__1), X__4 *100, X__4),
         X__5 = ifelse(grepl("%", X__1), X__5 *100, X__5)) %>%
  mutate_at(vars(2:5), funs(formatC(as.numeric(.),digits = 2, format = "f"))) %>%
  mutate_at(vars(2:5), funs(replace(., . == " NA", ""))) %>%
  add_row(X__2 = "AUD", X__3 = "DEP", X__4 = "OUD", X__5 = "OTHER", .before = 3)
...
sim_read <- function(x){
  dt <- read_excel(files[x], col_names = TRUE, range = "A17:DA129")
  dt <- dt %>%
    mutate(file_name = files[x],
           Experiment = sub(".*_(.*)\\.xls.*", "\\1", file_name),
           Experiment = replace(Experiment, Experiment == "bc", "Base Case"),
...
bc <- read_excel(files[grep("bc", files)], col_names = paste("X",1:2, sep = "__"), range = "A87:B129")
